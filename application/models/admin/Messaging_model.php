...
class Messaging_model extends CI_Model
{
    ...
    public function getTemplateByID($id) {
        $q = $this->db->get_where('sms_templates', array('id' => $id), 1);
        if( $q->num_rows() > 0 ) {
            return $q->row();
        }
        return FALSE;
    ...
    public function updateTemplate($id, $data = array()) {
        if($this->db->update('sms_templates', $data, array('id' => $id))) {
            return true;
        }
        return false;
    }
    public function SaveMessageTemplate($data){
        ...
        if ($this->db->insert('sms_templates', $data)) {
            return true;
        }
        return false;
    }
   ...
   public function getManualSMSTemplateListSelect2($searchTerm, $type) {
        if (!empty($searchTerm)) {
            ...
            $fetched_records = $this->db->get('sms_templates');
    ...
    public function deletetemplate($id) {
        if ($this->db->delete('sms_templates', array('id' => $id))) {
            return true;
        }
        return FALSE;
    }
