<?php
	class Example_model extends CI_Model{
		...
		public function get_users_for_csv(){
			$this->db->where('is_admin', 0);
			$this->db->select('id, username, firstname, lastname, email, phone, created_at');
			$this->db->from('users');
			$query = $this->db->get();
			return $result = $query->result_array();
		}
	...
	//  Charts
		...
		{
			$this->db->select('count(*) as users,roles.admin_role_title');
			$this->db->join('roles','roles.admin_role_id = users.admin_role_id');
			$this->db->group_by('users.admin_role_id');
			return $this->db->get('users')->result_array();
		}
