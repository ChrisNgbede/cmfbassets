...
class Admin_roles extends MY_Controller
{
	...
	function access($id=""){
		...
		if(empty($data['record'])){
			$this->session->set_flashdata('errors', 'Role not found!');
			redirect(base_url('admin/admin_roles'));
		}
		$data['access']= $this->admin_roles->get_access($id);
		$modules = $this->admin_roles->get_modules();
		
		foreach($modules as &$module){
			$methods = [];
			$controller_path = APPPATH.'controllers/admin/'.ucfirst($module['controller_name']).'.php';
			if(file_exists($controller_path)){
				$content = file_get_contents($controller_path);
				// Regex to find public functions
				preg_match_all('/public\s+function\s+([a-zA-Z0-9_]+)\s*\(/i', $content, $matches);
				if(!empty($matches[1])){
					$raw_methods = array_unique($matches[1]);
					$exclude = ['__construct', 'get_instance'];
					foreach($raw_methods as $m){
						if(in_array($m, $exclude)) continue;
						
						// Format name: underscores/hyphens to spaces, then split CamelCase
						$formatted = str_replace(['_', '-'], ' ', $m);
						$formatted = preg_replace('/(?<!^)[A-Z]/', ' $0', $formatted);
						$formatted = ucwords(trim($formatted));
			...
			$module['discovered_methods'] = $methods;
		}
	...
	function set_role_designation(){
		...
		$allowed_columns = [
			'is_asset_initiator', 'is_asset_approver', 'is_asset_tagger',
			'is_collateral_initiator', 'is_collateral_approver', 'is_collateral_tagger'
		];

		if(in_array($column, $allowed_columns) && $id){
			$data = [$column => $status];
