...
class Push extends MY_Controller {
	...
	public function add_item() {
		...
		if(isset($_POST['btn_add_item'])) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$id = $this->Push_model->insert_and_return_id('posts', $insert_data);
				if (!empty($id)) {
					$messages = array();
					...
					$query = $this->Push_model->get_data('push_subscribers')->result();
	...
	public function delete_item() {
		if ($this->Push_model->delete_record('posts', $this->input->post('id'))) {
			echo '1';
		} else {
			echo '0';
		}
	...
	public function push_subscription() {
		...
		switch ($method) {
			case 'POST':
				...
				//filter out bad data
				$myQuery = $this->db->query("SELECT * FROM push_subscribers WHERE endpoint = '".$this->input->post('endpoint')."'");
				try{
					...
					if(empty($result)) {
						...
						if ($this->Push_model->insert('push_subscribers', $insert_data)) {
			...
			case 'PUT':
				...
				try{
					...
					if($result[0]->id !== NULL) {
						...
						if ($this->Push_model->update_record('push_subscribers', $result[0]->id, $insert_data)) {
			...
			case 'DELETE':
				...
				try{
					...
					if(!empty($result[0]->id)) {
						...
						if ($this->Push_model->delete_record('push_subscribers', $result[0]->id)) {
	...
	public function send_push_notification() {
		...
		// but in reality, you'll get this information in your database
		// because you already stored it (cf. push_subscription.php)
		$query = $this->admin_model->get_data('push_subscribers')->result();
		foreach($query as $row) {
